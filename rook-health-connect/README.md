...
The `RookHealthConnectEnvironment` enum allows to quickly configure the behaviour of **rook-health-connect**, e.g. the api
...
To get permission of usage call `RookHealthConnectAuthorization.authorize` and provide:
...
```kotlin
val rookHealthConnectManager = RookHealthConnectManager(
    context = context,
    environment = RookHealthConnectEnvironment.SANDBOX,
    logLevel = "ADVANCED",
)
